...
export default function EditVaccination({ vaccination, newDate, setNewDate, handleVaccinationChange, handleStatusChange, handleVaccinationDateChange, handleRemoveDate, handleAddDate, handleRemoveVaccination, showTakenDateSelection, setShowTakenDateSelection, showPlannedDateSelection, setShowPlannedDateSelection }) {
	return (
		<div key={vaccination.id} className="flex flex-col mb-4 bg-white drop-shadow-lg rounded-md p-2">
				...
				{vaccination.datesTaken.map((date, index) => (
					<li key={index} className="font-normal">
						<input
							...
							className="bg-white drop-shadow-lg rounded-md p-2 mb-2"
						/>
						<button onClick={() => handleRemoveDate('datesTaken', vaccination.id, index)} className="bg-[#fff4f9] drop-shadow-lg rounded-md p-2 m-2">Remove Date</button>
					</li>
				))}
				...
				{vaccination.futureDates.map((date, index) => (
					<li key={index} className="font-normal">
						<input
							...
							className="bg-white drop-shadow-lg rounded-md p-2 mb-2"
						/>
						<button onClick={() => handleRemoveDate('futureDates', vaccination.id, index)} className="bg-[#fff4f9] drop-shadow-lg rounded-md p-2 m-2">Remove Date</button>
					</li>
				))}
			...
			<button onClick={() => handleRemoveVaccination(vaccination.id)} className="bg-[#fff4f9] drop-shadow-lg rounded-md p-2 mb-2">Remove Vaccination</button>
		</div>
	);
}